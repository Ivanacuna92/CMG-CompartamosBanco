...
db.run(`
  CREATE TABLE IF NOT EXISTS esac_messages (
...
export async function processChatMessageSQLite(req, res) {
  try {
    ...
    db.run(
      `INSERT INTO esac_messages (uuid, message, role, contract)
       VALUES (?, ?, 'Cliente', NULL)`,
      [userId, message],
      (err) => {
...
export async function newChatSessionSQLite(req, res) {
  try {
    ...
    if (oldId) {
      ...
      db.run(`DELETE FROM esac_messages WHERE uuid = ?`, [oldId], (err) => {
